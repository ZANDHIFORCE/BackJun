...
def solution(maze):
    ...
    mildstone = [10000000000000000000000000000000000000000000000000]
    copy_maze = [x[:] for x in maze]
    DFS(red_c, blue_c, red_goal, blue_goal, 0, mildstone, copy_maze)
    if mildstone[0] == 10000000000000000000000000000000000000000000000000:
        mildstone[0] = 0
    return mildstone[0]
...
def DFS(red_c, blue_c, red_goal, blue_goal, count, mildstone, maze):
    ...
    if is_in(red_c, maze) and is_in(blue_c, maze):
        ri, rj = red_c
        bi, bj = blue_c
        red_validate = [0,3,4,8]
        blue_validate = [0,3,4,7]
        #유효성 검사
...
maze = [[1, 0, 0, 2], [0, 5, 5, 0], [0, 5, 5, 0], [4, 0, 0, 3]]
print(solution(maze))